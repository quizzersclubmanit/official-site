const final=["Archit Chibbar", "Aaditya Jain", "Hritika Sharma", "Paras Dandwani", "Shraddha Bisen", "Mudit Gokhale", "Pankaj Gaur", "Tanishq Singhai", "Nitesh Sharma", "Lalit Singh Rajput", "Siddharth Pankar", "Princy Jain", "Satwik Sharma", "Utkarsh Shrivastava", "Shanul Nema", "Mohammad Anas", "dummy","dummy","dummy"];
const prefinal=["Pratham Khare", "Abhishek Sinha", "Manaswi Sathe", "Aman Malviya", "Sakshi Talreja", "Kalyan Roy", "Imrah Ravish","Satyam Chaturvedi","Riya Jain", "Akriti Yadav", "Shivi Jain","Arnav Kulkarni","Rajat Kumar","Rajesh Kumar","Sambhav Bhatt","Yash Gupta","Chirag Gupta","Aman Gupta", "Aditya Dhakad"];
const colors=["#011627","#4b2977","#771543","#0e5e83"];
console.log(prefinal.length);
let cat=true;
window.addEventListener("load", (e)=>{
    document.getElementById("_8").scrollIntoView({inline:'center', block:'center', behavior: "smooth"});
    ...
    document.getElementById("toggler").addEventListener("click", e=>{
        ...
        if(cat){
            makeUp("Final", final);
            document.querySelector("#team-head > h1 > span").innerText="final Years";
            cat=false;
        }else{
            makeUp("Prefinal", prefinal);
            document.querySelector("#team-head > h1 > span").innerText="Pre-final Years";
            cat=true;
        }
    })
...
function makeUp(category, arr){
    ...
    document.querySelectorAll(".parent > img").forEach(img=>{
        img.setAttribute('src', './'+category+'/'+arr[i].toLowerCase().split(' ').join("%20")+".png");
        i++;
    })
